(() => {
  ...
  function renderDashboard() {
    ...
    elements.datasetMeta.textContent = `${dataset.questions.length.toLocaleString()}문제 · ${formatExportedAt(dataset.exportedAt)} 내보냄`;
    updateStats();
    updateMatchedCount();
    showView("dashboard");
  }
  ...
  function startStudy(statusMode) {
    ...
    const requestedCount = elements.countFilter.value;
  ...
  async function importBackup(file) {
    ...
    populateFilters();
    renderDashboard();
    showToast(`${validated.questions.length.toLocaleString()}문제를 가져왔습니다.`);
  }
  ...
  function bindEvents() {
    ...
    elements.leaveStudy.addEventListener("click", () => {
      if (window.confirm("현재 풀이를 나갈까요? 이미 답한 기록은 저장되어 있습니다.")) renderDashboard();
    });
